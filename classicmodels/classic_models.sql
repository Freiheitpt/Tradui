...
--
-- Host: 127.0.0.1
-- Erstellungszeit: 29. Nov 2018 um 06:21
...
--
-- Stellvertreter-Struktur des Views `customer_order_volume`
-- (Siehe unten für die tatsächliche Ansicht)
--
CREATE TABLE `customer_order_volume` (
`customerNumber` int(11)
,`country` varchar(50)
,`orderVolume` decimal(32,0)
);
...
--
-- Stellvertreter-Struktur des Views `orders_by_country`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Struktur des Views `customer_order_volume`
--
DROP TABLE IF EXISTS `customer_order_volume`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_order_volume`  AS  select `c`.`customerNumber` AS `customerNumber`,`c`.`country` AS `country`,sum(`od`.`quantityOrdered`) AS `orderVolume` from ((`customers` `c` join `orders` `o` on((`o`.`customerNumber` = `c`.`customerNumber`))) join `orderdetails` `od` on((`od`.`orderNumber` = `o`.`orderNumber`))) group by `c`.`customerNumber` ;

-- --------------------------------------------------------

--
-- Struktur des Views `orders_by_country`
--
DROP TABLE IF EXISTS `orders_by_country`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `orders_by_country`  AS  select `c`.`country` AS `country`,sum(`od`.`quantityOrdered`) AS `total_orders` from ((`customers` `c` join `orders` `o` on((`o`.`customerNumber` = `c`.`customerNumber`))) join `orderdetails` `od` on((`od`.`orderNumber` = `o`.`orderNumber`))) group by `c`.`country` ;
